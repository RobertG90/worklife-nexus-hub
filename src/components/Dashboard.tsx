...
export function Dashboard({ onSectionChange }: DashboardProps) {
  ...
  return (
    <div className="space-y-5 sm:space-y-6 md:space-y-8">
      {/* Welcome Section */}
      <div className="bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl md:rounded-2xl p-5 md:p-8 text-white">
        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2">Welcome back, John! 👋</h2>
        <p className="text-blue-100 text-sm md:text-lg">
          Ready to optimize your workday? Everything you need is just a click away.
        </p>
        <div className="mt-4 md:mt-6 flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-6">
          <div className="flex items-center space-x-2">
            <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5" />
            <span className="text-xs sm:text-sm">98% automation rate</span>
          </div>
          <div className="flex items-center space-x-2">
      ...
      <Card className="p-4 md:p-6">
        <h3 className="text-base md:text-lg font-semibold text-gray-900 mb-4 md:mb-6">Recent Activity</h3>
        {isActivitiesLoading ? (
          <div className="text-center py-6 md:py-8 text-gray-500">Loading activities...</div>
          ...
          <div className="space-y-3 md:space-y-4">
            {recentActivities.map((activity: any) => (
              <div
                ...
                className="flex items-center justify-between p-3 md:p-4 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors"
              >
                ...
                <Badge 
                  ...
                  className={
                    activity.status === 'approved' ? 'bg-green-100 text-green-800 text-xs' :
                    activity.status === 'confirmed' ? 'bg-blue-100 text-blue-800 text-xs' :
      ...
      <Card className="p-4 md:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {quickActions.map((action, index) => (
            ...
            >
              <button 
                className="w-full p-3 md:p-4 border border-gray-200 rounded-lg hover:bg-gray-50 text-left transition-colors"
              >
                <div className={`${action.color} mb-1 md:mb-2 text-lg sm:text-xl`}>{action.emoji}</div>
